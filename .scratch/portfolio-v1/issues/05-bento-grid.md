...
`npm run typecheck`, `npm run test` (48 Vitest assertions, untouched by this ticket), and the full `npm run test:floor` (bundle size, Lighthouse, and all five Playwright specs including the new one) pass: LCP 1504ms, Lighthouse accessibility 100, 0KB initial-route JS.

Reviewed with `/code-review` (Standards axis; the Spec sub-agent hit a session rate limit and was reviewed manually against the ticket and spec instead). One hard violation: the Experience cell used `&mdash;` (an em dash), which the user's global instructions forbid outright; fixed to a plain `-`. Two judgement calls, both left as-is: the seven cell `<div>`s repeat a `clay col-span-N ... md:col-span-M` shape that could be pulled into a shared component, but ticket 08 replaces this markup with content-collection-driven rendering shortly, so extracting a component now risks building the wrong shape from placeholder data; and this Comments section wraps multiple sentences per line rather than one-per-line, matching every prior ticket's Comments section exactly, so keeping it consistent with that established precedent outweighs the letter of the global Markdown rule here.
